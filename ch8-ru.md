...
`withdraw` начнёт крутить носом и вернёт `Maybe(null)`, если у нас заночатся деньги. Эта функция также передает свою непостоянство и не оставляет нам выбора, кроме как использовать `map` после нее. Разница в том, что `null` был здесь специально. Вместо `Maybe(String)`, мы получаем `Maybe(null)` назад как сигнал об ошибке и наше приложение эффективно останавливает свою работу. Это важно отметить: если `withdraw` падает с ошибкой, то `map` остановит остальную часть нашего вычисления, так как она никогда не запускает отображенные функции, а именно `finishTransaction`. Это именно то поведение, которое мы предпочитаем не обновлять нашу бухгалтерскую книгу или не показывать новый баланс, если мы не смогли успешно вывести средства.
...
Одна вещь, которую люди часто упускают, заключается в том, что всегда будет конец линии; какая-то эффектная функция, которая посылает JSON или печатает на экране, или изменяет нашу файловую систему, или что у вас есть. Мы не можем вернуть результат с `return`, мы должны запустить какую-то функцию или другую, чтобы отправить его в мир. Мы можем сказать это в духе дзен буддистских коанов: "если программа не имеет наблюдаемого эффекта, она вообще работает?". Правильно ли он работает для собственного удовлетворения? Я подозреваю, что это просто сжигает некоторую энергию и возвращается спать...
